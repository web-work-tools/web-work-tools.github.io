...
* [#indieweb-dev on DIDs 2019-03-02-chat log](https://freenode.logbot.info/indieweb-dev/20190302)
  ...
  * 16:16 **aaronpk** -DIDs lost me when they make the assumption that blockchains are the only resolution mechanism
  ...
  * 16:19 **sandhawke** -But I agree there's a culture clash
  * 16:19 **aaronpk** -From what I understand they have no interest in relying on DNS so that takes them completely out of the realm of working with current browsers or anything like IndieAuth
  * 16:20 **aaronpk** -in theory you could make a dns resolver method for did and use domain names as the identifier and get close to something like IndieAuth
  * 16:20 **sandhawke** -Right.
  * 16:21 **sandhawke** -And it doesn't matter if they want that
  * 16:21 **sandhawke** -The end-users benefit
  * 16:22 **aaronpk** -The other thing missing from DID is the authorization framework that comes with IndieAuth since it's built on OAuth 2.0
  * 16:22 **sandhawke** -They're working on DID-Auth, trying to figure out what that means. There are two papers about that this weekend, one from Microsoft.
  * 16:23 **aaronpk** -The DID and OIDC groups weren't even getting along very well and while OIDC is way overkill for what we use IndieAuth for, its still more similar to IndieAuth than blockchain stuff is
  * 16:24 **sandhawke** -I agree re "way overkill", so I want to show how simple it can be. They're not the first "way overkill" people in identity / authn / authz.
  * 16:25 sandhawke -I mean -- maybe I should just go watch **netflix** -- this might be a silly waste of time, but I'm here, and it's a lot of smart and motivate people, so I can't help but wonder about trying to steer them a bit.
  * 16:26 **aaronpk** -I would be very interested in what you come up with. I haven't had much luck in any of these conversations so far
  * 16:27 **sandhawke** -fair enough :-) Is indieauth.spec.indieweb.org pretty much up-to-date with your thinking?
  * 16:28 **sandhawke** -( sad not to see a change log from https://w3.org/TR/2018/NOTE-indieauth-20180123 )
  ...
  * 16:52 **sandhawke** - I don't understand the value of public apps being identified
  * 16:52 **sandhawke** - (good video)
  * 16:59 **sandhawke** - I guess there's some attack it's intended prevent.
  * 17:01 **aaronpk** - Yeah that's from OAuth. Prevents redirect interception by registering the redirect URL, and also helps the user not get phished
  * 20:28 **jacky** - tbh as much as I'm for the whole DiD and what not
  ...
  * 20:55 **jacky** - like if I can't say it out loud, it's incredibly hard to share
  * 20:55 **jacky** - sure, I can just share a link
  * 21:00 **[tantek]** - speechfriendly++
  * 21:00 **Loqi** - speechfriendly has 1 karma over the last year
  * 21:05 **swentel** - [tantek], yeah, I figured that it might be a useful link for people looking for my 'main' feed
  * 21:06 **swentel** - also because currently in aperture, rel="feed" is ignored. So if people search feeds on realize.be, they will get microformats, but that's on my homepage, which is kind of useless
  * 21:06 **swentel** - which makes me wonder whether I should remove microformat markup on the homepage maybe
  * 21:08 **swentel** - (to be more specific, xray underneath ignores rel="feed", but that's technical detail)
  ...
  * 21:12 **swentel** - hmm, GWG opened an issue for me indigenous, there's probably a link there, let me check
  * 21:12 **swentel** - indieweb/indieauth #31
  * 21:12 **Loqi** - [dshanske] #31 Returning Profile Data
  * 21:13 **jacky** - nice
  ...
  * 21:13 **aaronpk** - hrm I don't think it should be "nickname"
  * 21:13 **Zegnat** - Hardly “should be”
  * 21:14 **jacky** - that makes sense
  * 21:14 **jacky** - b/c name is so vague
  * 21:14 **jacky** - vs 'username', 'nickname', 'given name'
  * 21:14 **aaronpk** - display name is more accurate
  ...
  * 21:16 **GWG** - If someone gives it another +1 I'll go make a PR
  * 21:16 **swentel** - Hmm, I like name tbh :)
  * 21:16 **aaronpk** - I also don't necessarily think "name" is a bad property for this, just need to make it clear that there is no expectation of a "real name" or "full name"
  * 21:16 **swentel** - as it's consistent then
  ...
  * 21:22 **[tantek]** - that's a hypothetical concern from which no one has ever documented any actual harm ever
  * 21:22 **GWG** - swentel, link to that under Micropub experimental as well.
  * 21:23 **swentel** - GWG, on micropub extensions?
  * 21:23 **GWG** - Swentel, yes, excuse me.
  * 21:23 **swentel** - Ok, maybe it's better to move it there completely
  * 21:24 **[tantek]** - aaronpk is correct, and opendid connect even there screwed up (inserted Western-centric assumptions by I'm sure the 100% Western white male openid connect "designers) by renaming vCard's "additional name" into "middle_name".
  * 21:24 **[tantek]** - openidconnect--
  * 21:24 **Loqi** - openidconnect has -1 karma over the last year
  * 21:24 **GWG** - Do we have a theoretical use case for more than minimal data yet?
  * 21:24 **[tantek]** - westerncentricassumptions--
  * 21:24 **Loqi** - westerncentricassumptions has -1 karma over the last year
  * 21:26 **GWG** - I return name, url, and photo because that was the minimum needed for a client to display. I am happy to return more behind a profile scope the day someone wants it
  * 21:26 **[tantek]** - (note: " middle" is a layout/order specific label, not a semantic. in the past these same kinds of 100% Western white male format designers would have "first_name" "middle_name" "last_name", there's plenty of examples of bad old dead formats with that, programming examples in various "5 minutes to make your own social network!" type stuff etc.) 
  * 21:26 **aaronpk** - GWG: I'd hold off on anything further with this until we get more publishers and consumers of it
  * 21:27 **jacky** - r^
  * 21:27 **GWG** - aaronpk, I planned on it
  * 21:27 **GWG** - I just was curious about uses
  * 21:27 **GWG** - I have bigger problems right now
  * 21:27 **[tantek]** - swentel - I think if you put rel="alternate" on the link to your h-feed page then it may be discoverable. I believe rel=feed was deprecated (even dropped by WHATWG HTML for lack of interest / implementations)
  * 21:28 **swentel** - [tantek], oh, interesting, I'll give that a try yes!
  * 21:28 **GWG** - Really? I just added a bunch of rel feeds
  * 21:28 **GWG** - I can't keep up
  * 21:29 **GWG** - I thought alternate was an alternative view of the same page and feed was a different page
  * 21:29 **GWG** - What is rel-feed?
  * 21:29 **Loqi** - rel-feed is the standard for linking to multiple (potentially alternative) h-feeds from a site's homepage using the code rel="feed" on those links indieweb.org/rel-feed
  * 21:30 **jacky** - yeah same
  * 21:30 **jacky** - re: G WG
  * 21:31 **[tantek]** - no feed discovery is done via rel=alternate type="insert your feed mime type here"
  * 21:31 **[tantek]** - like since forever
  * 21:31 **GWG** - That's the way I am using and consuming it
  * 21:31 **aaronpk** - whoa swentel indieweb.org/Micropub-extensions#Location_checkin_information
  * 21:31 **[tantek]** - folks here have been trying to get rel=feed off the ground again but TBH I'm unsure about it's feasibility
  * 21:31 **GWG** - What is feed discovery?
  * 21:31 **Loqi** - feed discovery is a way to, given someone's home page, discover their feed or feeds that they publish indieweb.org/feed_discovery
  * 21:31 **aaronpk** - that's like an embedded h-card in a geo URL?
  * 21:31 **aaronpk** - URI
  * 21:31 **swentel** - aaronpk, yeah, works great .. (at least for me)
  * 21:32 **GWG** - aaronpk, yes it is
  * 21:32 **[tantek]** - aaronpk you had it right 🙂
  * 21:32 **swentel** - I can post checkins and geocaches now from indigenous, with pictures, all in one go :)
  * 21:32 **swentel** - it's GREAT ;)
